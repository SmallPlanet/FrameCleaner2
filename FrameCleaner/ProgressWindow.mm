...
- (void) preloadProgressBar {
    [self startProgressBarWithMessage:@"SPECIAL_PRELOAD_MESSAGE"];
}

- (void) continueProgressBarMessage:(NSString *)message withValue:(float)value {
	ProgressWindow * pw = [self sharedProgressWindow];
...
- (void) drawRect: (NSRect) rect {
	...
	if([message isEqualToString:@"SPECIAL_PRELOAD_MESSAGE"]) {
		...
		NSMutableDictionary *bold12White = [NSMutableDictionary dictionary];
		NSRect frame = [self bounds];
		NSSize size;
		CGRect cgrect;
		
		[bold12White setObject: [NSFont systemFontOfSize:14] forKey: NSFontAttributeName];
		...
		strokeRoundedRect((CGContext *)[[NSGraphicsContext currentContext] graphicsPort], cgrect, 4, 4);
