...
{
    return [[NSOffsetData alloc] initWithBytesNoCopy:bytes length:length freeWhenDone:YES];
}

+ (id)dataWithBytesNoCopy:(void *)bytes length:(NSUInteger)length freeWhenDone:(BOOL)b
{
    return [[NSOffsetData alloc] initWithBytesNoCopy:bytes length:length freeWhenDone:b];
}
...
{
    data = [NSData dataWithBytesNoCopy:replacementBytes length:replacementLength freeWhenDone:YES];
...
- (NSOffsetData *) cczInflate
{
	...
	if( header->sig[0] != 'C' || header->sig[1] != 'C' || header->sig[2] != 'Z' || header->sig[3] != '!' ) {
		...
		return [[NSOffsetData alloc] init];
	}
