...
//    
//    CGRect frame = self.cgrect;
//    frame = (CGRectIsNull(frame) ? pointRect : CGRectUnion(frame, pointRect));
//    numberOfPoints++;
//}
...
- (BOOL) overlaps:(SPBorderedView *)region
{
    ...
    if (CGRectContainsPoint([region cgrect], p1) && CGRectContainsPoint([region cgrect], p2)) return YES;
    if (CGRectContainsPoint([region cgrect], p2) && CGRectContainsPoint([region cgrect], p3)) return YES;
    if (CGRectContainsPoint([region cgrect], p3) && CGRectContainsPoint([region cgrect], p4)) return YES;
    if (CGRectContainsPoint([region cgrect], p4) && CGRectContainsPoint([region cgrect], p1)) return YES;
    return NO;
}
